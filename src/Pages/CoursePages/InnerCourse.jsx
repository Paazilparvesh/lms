...
function InnerCourse() {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://lms.thirdvizion.com/api/studentprogress/", {
        params: {
          email: student_email,
          course_id: id,
        },
  ...
  async function handlePayment() {
    ...
    try {
      const response = await axios.post(
        "https://lms.thirdvizion.com/api/courseenrollment/",
        {
          email: student_email,
          name: name,
          number: number,
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            const verifyRes = await axios.post(
              "https://lms.thirdvizion.com/api/success/",
              {
                razorpay_payment_id: response.razorpay_payment_id,
                razorpay_order_id: response.razorpay_order_id,
                razorpay_signature: response.razorpay_signature,
            ...
            if (verifyRes.status === 200) {
              await axios.get(
                "https://lms.thirdvizion.com/api/courseenrollmentprogress/",
                {
                  params: {
                    course_id: Number(id),
                    email: student_email,
    ...
    <>
      ...
      >
        ...
        {pay && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center px-4">
            <div
              className={`w-full max-w-lg p-8 rounded-lg shadow-2xl relative z-20 ${
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={name}
                    onChange={(e) => SetName(e.target.value)}
                    className={`w-full p-2 rounded border focus:outline-none focus:ring-2 focus:ring-orange-500 ${
                      isDarkMode
